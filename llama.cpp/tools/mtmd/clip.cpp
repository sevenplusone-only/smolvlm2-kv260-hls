    ...
    clip_ctx(clip_context_params & ctx_params) {
        ...
        if (ctx_params.use_gpu) {
            ...
            if (!backend) {
                ...
                if (!backend) {
                    // ACCEL is shared by FPGA/NPU backends (XRT, etc.) and by BLAS. We can't
                    // use init_by_type(ACCEL) because it returns whichever was registered first
                    // (usually BLAS), which is a CPU SIMD wrapper rather than a real accelerator.
                    // Iterate and skip BLAS so a true accelerator wins when present.
                    for (size_t i = 0; i < ggml_backend_dev_count(); ++i) {
                        ...
                        if (dn && std::string(dn) == "BLAS") continue;
